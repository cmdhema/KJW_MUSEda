...
public class PhotoProfileUpRequest extends MultiPartNetworkRequest<UserData> {

	private UserData data;
	
	public PhotoProfileUpRequest(UserData list, UserData result) {
		super(result);
		this.data = list;
	}
	...
	public MultipartEntity getMultipartEntity() {
		...
		try {
			...
			multiEntity.addPart("index", new StringBody(data.photoIndex+"", chars));
...
//		Log.i("LoginRequest", jsonBuf.toString());
		...
		try {
			...
			result.resultPhotoThumbPath = (dataObject.getString("thumb_url"));
			
			if(result.errorCode != 0)
				return false;
			
		} catch (JSONException je) {
			Log.e("PhotoProfileUpRequest", "JSON파싱중 에러 발생", je);
		}
